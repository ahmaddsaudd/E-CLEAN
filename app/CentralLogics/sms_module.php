...
class SMS_module
{
    ...
    {
         // iSmart SMS
         $config = self::get_settings('ismart_sms');
         if (isset($config) && $config['status'] == 1) {
             return self::ismart_sms($receiver, $otp, $message);
         }
    ...
    public static function ismart_sms($receiver, $otp, $message = null): string
    {
        $config = self::get_settings('ismart_sms');
        $response = 'error';
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            // Format the receiver for the iSmart SMS API
            $receiver = str_replace("+", "", $receiver);

            $url = 'https://www.ismartsms.net/iBulkSMS/HttpWS/SMSDynamicAPI.aspx?' . http_build_query([
                'UserId' => $config['user_id'],
                'Password' => $config['password'],
                'MobileNo' => $receiver,
                'Message' => $message,
                'Lang' => $config['lang'] ?? 0,
                'FLashSMS' => $config['flash_sms'] ?? 'N'
            ]);
